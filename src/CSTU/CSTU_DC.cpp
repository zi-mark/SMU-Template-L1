#include "drivercontrol.h"
#include "definer.h"
#include "robot-config.h"
#include "SMU_Lib/chasis.h"
#include "SMU_Lib/functional.h"
#include "initialize.h"

#ifdef CSTU
    int lv, rv;
    bool save;

    #if CSTU == 1 || CSTU == 5
    void DC(){
    Stop(coast);
    Brain.Screen.clearScreen(black);
        while(1){

            #if DriversHabit == 1
            lv = Con.Axis3.position() + Con.Axis4.position();
            rv = Con.Axis3.position() - Con.Axis4.position();

            // if(Con.ButtonUp.pressing()) Lift_SpinTo(150, 100, 0);
            // else if(Con.ButtonX.pressing()) Lift_SpinTo(30, 100, 0);
            // else if(abs(Con.Axis2.position()) > 4){
            //     if(Lift_AverPosition() <= 100 || abs(Con.Axis1.position()) >= 40) Lift_Spin(Con.Axis2.position());
            //     else if(Con.Axis2.position() <= 4) Lift_Spin(Con.Axis2.position());
            //     else Lift_Stop() ;
            // }
            // else if(Lift_IsDone()) Lift_Stop();

            if(abs(Con.Axis2.position()) >= 5) Lift_Spin(Con.Axis2.position());
            else Lift_Stop(hold);

            if(Con.ButtonL1.pressing()) Hook.set(1);
            if(Con.ButtonL2.pressing()) Hook.set(0);

            #elif DriversHabit == 2
            lv = Con.Axis3.position() + Con.Axis1.position();
            rv = Con.Axis3.position() - Con.Axis1.position();

            if(Con.ButtonA.pressing()) Hook.set(1);
            else if(Con.ButtonB.pressing()) Hook.set(0);

            if(Con.ButtonUp.pressing()) Lift_SpinTo(150, 100, 0);
            else if(Con.ButtonDown.pressing()) Lift_SpinTo(30, 100, 0);
            else if(Con.ButtonL1.pressing()) Lift_Spin(100);
            else if(Con.ButtonL2.pressing()) Lift_Spin(-100);
            else if(Lift_IsDone()) Lift_Stop(hold);

            #endif

            if(abs(lv) < 5) lv = 0;
            if(abs(rv) < 5) rv = 0;
            SpinLR(lv, rv);

            if(Con.ButtonR1.pressing()) Suck_Spin(100);
            else if(Con.ButtonR2.pressing()) Suck_Spin(-100);
            else Suck_Stop(coast);
            
            if(Lift_AverPosition() > 130) HandR.set(1);
            else if(Con.ButtonB.pressing()) HandR.set(1);
            else HandR.set(0);

            if(Con.ButtonY.pressing()) HandL.set(1);
            else HandL.set(0);

        }
    }
    #elif CSTU == 2
    void DC(){
    Stop(coast);
        while(1){

            #if DriversHabit == 1
            lv = Con.Axis3.position() + Con.Axis4.position();
            rv = Con.Axis3.position() - Con.Axis4.position();

            if(Con.ButtonUp.pressing()) Lift_SpinTo(370, 100, 0);
            else if(Con.ButtonDown.pressing()) Lift_SpinTo(0, 100, 0);
            else if(abs(Con.Axis2.position()) > 4) Lift_Spin(Con.Axis2.position());
            else if(Lift_IsDone()) Lift_Stop();

            if(Con.ButtonL1.pressing()) Hook.set(1);
            if(Con.ButtonL2.pressing()) Hook.set(0);

            #elif DriversHabit == 2
            lv = Con.Axis3.position() + Con.Axis1.position();
            rv = Con.Axis3.position() - Con.Axis1.position();

            if(Con.ButtonA.pressing()) Hook.set(1);
            else if(Con.ButtonB.pressing()) Hook.set(0);

            if(Con.ButtonUp.pressing()) Lift_SpinTo(370, 100, 0);
            else if(Con.ButtonDown.pressing()) Lift_SpinTo(0, 100, 0);
            else if(Con.ButtonL1.pressing()) Lift_Spin(100);
            else if(Con.ButtonL2.pressing()) Lift_Spin(-100);
            else if(Lift_IsDone()) Lift_Stop(hold);

            #endif

            if(abs(lv) < 5) lv = 0;
            if(abs(rv) < 5) rv = 0;
            SpinLR(lv, rv);

            if(Con.ButtonR1.pressing()) Suck_Spin(100);
            else if(Con.ButtonR2.pressing()) Suck_Spin(-100);
            else Suck_Stop();

            if(Con.ButtonX.pressing()) CLSelector.set(1);
            else if(!Con.ButtonB.pressing()) ColorSelecting(ProgramChoice);
            else CLSelector.set(0);

        }
    }

    #elif CSTU == 6

    void DC(){
        Stop(coast);
        Brain.Screen.clearScreen(black);
            while(1){
    
                #if DriversHabit == 1
                lv = Con.Axis3.position() + Con.Axis4.position();
                rv = Con.Axis3.position() - Con.Axis4.position();
    
                if(abs(Con.Axis2.position()) >= 5) Lift_Spin(Con.Axis2.position());
                else Lift_Stop(hold);
    
                if(Con.ButtonL1.pressing()){
                    HookP.set(0);
                    HookR.set(1);
                }
                if(Con.ButtonL2.pressing()){
                    HookP.set(1);
                    HookR.set(0);
                }

                #elif DriversHabit == 2
                lv = Con.Axis3.position() + Con.Axis1.position();
                rv = Con.Axis3.position() - Con.Axis1.position();
    
                if(Con.ButtonA.pressing()) {
                    HookP.set(0);
                    HookR.set(1);
                }
                else if(Con.ButtonB.pressing()) {
                    HookP.set(1);
                    HookR.set(0);
                }
                if(Con.ButtonUp.pressing()) Lift_SpinTo(150, 100, 0);
                else if(Con.ButtonDown.pressing()) Lift_SpinTo(30, 100, 0);
                else if(Con.ButtonL1.pressing()) Lift_Spin(100);
                else if(Con.ButtonL2.pressing()) Lift_Spin(-100);
                else if(Lift_IsDone()) Lift_Stop(hold);
    
                #endif
    
                if(abs(lv) < 5) lv = 0;
                if(abs(rv) < 5) rv = 0;
                SpinLR(lv, rv);
    
                if(Con.ButtonR1.pressing()) Suck_Spin(100);
                else if(Con.ButtonR2.pressing()) Suck_Spin(-100);
                else Suck_Stop(coast);
                
                if(Con.ButtonY.pressing()) {
                    HandRP.set(1);
                    HandRR.set(0);
                }
                else{
                    HandRP.set(0);
                    HandRR.set(1);
                }
    
                // if(Con.ButtonY.pressing()) HandL.set(1);
                // else HandL.set(0);
    
            }
        }
    
    #endif
#endif